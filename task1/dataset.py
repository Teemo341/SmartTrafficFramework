...
def preprocess_edge(edge_dir):
    ...
    #dges = [(int(o), int(d), float(l), translate_roadtype_to_capacity(c)) for _, (o, d, c, geo, l) in edges.iterrows()]
    
    edges = [{
        "Origin": int(o),
        "Destination": int(d),
        "length": float(l),
        "capacity": translate_roadtype_to_capacity(c)
    } for _, (o, d, c, geo, l) in edges.iterrows()]

    return edges

def preprocess_traj(traj_dir):
    trajs = pd.read_csv(traj_dir,nrows=1000)
    trajs = [
            {"VehicleID": vid,
             "TripID": tid,
