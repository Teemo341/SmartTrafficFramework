...
class traj_dataset(Dataset):
    def __init__(self, city='boston', data_dir='./data', simulation_num=1000000, history_num = 5, block_size=60, start_id = 0, weight_quantization_scale=None):
    ...
    def sample_jinan_data(self, city, path='./data', max_connection=9):
        ...
        self.traj = []
        self.time_step = []
        for tid in tqdm(range(self.start_id, self.simulation_num+self.start_id,1), desc=f'Transfering {city} points into trajectories'):
...
def custom_collate_fn(batch):
    # batch: [(trajectory, time_step, special_mask, adj_table)]
    dense_tensor0 = []
    dense_tensor1 = []
    dense_tensor2 = []
    sparse_tensors = []

    for traj, time_step, special_mask, adj_table in batch:
        dense_tensor0.append(traj)
        dense_tensor1.append(time_step)
        dense_tensor2.append(special_mask)
        sparse_tensors.append(adj_table)
    
    # 对稠密张量使用默认的堆叠方法
    dense_batch0 = torch.stack(dense_tensor0, dim=0)
    dense_batch1 = torch.stack(dense_tensor1, dim=0)
    dense_batch2 = torch.stack(dense_tensor2, dim=0)
    ...
    for i, sparse_tensor in enumerate(sparse_tensors):
        ...
        # 添加偏移量，保持稀疏矩阵的正确性
        batch_indices = torch.cat([torch.full((1, indices.shape[1]), i, dtype=torch.long), indices], dim=0)
        sparse_indices.append(batch_indices)
        sparse_values.append(values)
    
    # 将所有稀疏张量合并到一个新的稀疏张量中
    stacked_sparse_indices = torch.cat(sparse_indices, dim=1)
    stacked_sparse_values = torch.cat(sparse_values)
    # print(sparse_indices[0].shape, sparse_values[0].shape)
    # print(stacked_sparse_indices.shape, stacked_sparse_values.shape)
    # print(batch_size, *sparse_tensors[0].shape)
    sparse_batch = torch.sparse_coo_tensor(stacked_sparse_indices, stacked_sparse_values, size=(batch_size, *sparse_tensors[0].shape))
    
    return dense_batch0,  dense_batch1, dense_batch2, sparse_batch


# dataloader with randomize condition function
# boston has 500000 train cars, 10000 test cars
...
class traj_dataloader():
    def __init__(self, city='boston', data_dir='./data', test_data_dir = None,
        ...
        self.train_loader = DataLoader(traj_dataset(city=city, data_dir=data_dir, simulation_num=simulation_num, start_id=0, block_size=block_size), batch_size=batch_size, shuffle=shuffle, num_workers=num_workers)
        if test_data_dir is not None:
            self.test_loader = DataLoader(traj_dataset(city=city, data_dir=test_data_dir, simulation_num=test_simulation_num, start_id = simulation_num, block_size=block_size),batch_size=batch_size, shuffle=False, num_workers=num_workers)
        else:
...
if __name__ == '__main__':
    ...
    loader = traj_dataloader(city='jinan', data_dir='./data',
                             simulation_num=8000, test_simulation_num=1631, condition_num=5, block_size=60, capacity_scale=10, weight_quantization_scale=None, max_connection=9,
                             batch_size=256, shuffle=True, num_workers=8)
