...
def task3_presention(ratio=None):
    # load data
    ...
    if ratio is not None:
        dataloader.reduce_observation(ratio)
    ...
    print('finish forward, start calculate load')
    del model
    del dataloader

    # test_logits = np.load('task3/test_logits.npy')
    # test_trajectory = np.load('task3/test_trajectory.npy')
    ...
    print('calculate finish, start ploting')
...
def plot_volume(G, pos, volume_single, max_volume, fig_size=20, save_path=None):
    # G networkx graph
...
def make_video_from_frames(frames_dir, video_path):
    ...
    real_fig = plot_volume(G, pos, volume, max_volume, fig_size)
    volume = predic_load[20]
    pre_fig = plot_volume(G, pos, volume, max_volume, fig_size)
    ...
    return real_fig, pre_fig, fig ,'./videos/jinan/real/video.mp4'

if __name__ == '__main__':
    #save_cal_data()
    #plot_h()
    task3_presention(ratio=0.4)
    pass