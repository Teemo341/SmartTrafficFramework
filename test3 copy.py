...
def test_presention(num, observe_ratio,save_path=None):
    ...
    dataloader = SmartTrafficDataloader(dataset,batch_size=64,shuffle=True, num_workers=4,sampler=sampler)
    ...
    with torch.no_grad():
        for condition, time_step, special_mask, adj_table in tqdm(dataloader, desc='Testing', total=len(dataloader)):
            ...
            logits, loss = model(x, condition, adj_table, None, None, None, None)
            loss = torch.mean(loss)
        ...
        print("calculate volume from logit")
        ...
        real_path,predic_path =plot_presention(real_load,predic_load,observe_ratio,save_path=save_path)
        
    return  real_path ,predic_path


def plot_volume(G, pos, volume_single, max_volume, figure_size=20, save_path=None, return_cv = False):
    # G networkx graph
...
def plot_presention(real_load,predic_load,observe_ratio,save_path=None):
    ...
    make_volume_frames(G, pos, real_load, max_volume, fig_size, save_path = f'{save_path}/videos/real')
    make_volume_frames(G, pos, predic_load, max_volume, fig_size, save_path = f'{save_path}/videos/predic')
    make_video_from_frames(f'{save_path}/videos/real', f'{save_path}/videos/real.mp4')
    make_video_from_frames(f'{save_path}/videos/predic', f'{save_path}/videos/predic.mp4')
    return f'{save_path}/videos/real.mp4', f'{save_path}/videos/predic.mp4'

if __name__ == '__main__':
    #train_presention(0.5)
    test_presention(1000,0.5,save_path='./UI_element/task2')
    # plot_presention()
